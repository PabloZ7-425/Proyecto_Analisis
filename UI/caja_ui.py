...
class VentanaCaja(QWidget):
    ...
    def __init__(self, usuario_data):
        ...
        # --- NUEVO: Configuración para el desglose ---
    ...
    def crear_tab_apertura(self):
        tab = QWidget()
        layout_principal = QHBoxLayout() # Cambiado a Horizontal para el desglose
        layout_principal.setSpacing(25)
        layout_principal.setContentsMargins(15, 15, 15, 15)

        # --- SECCIÓN IZQUIERDA: DESGLOSE DE EFECTIVO ---
        grupo_desglose = QGroupBox("Conteo para Apertura")
        ...
        for i, den in enumerate(self.denominaciones):
            ...
            btn_menos = QPushButton("-"); btn_menos.setFixedSize(30, 30)
            btn_menos.clicked.connect(lambda ch, d=den: self.ajustar_conteo(d, -1))
            
            edit = QLineEdit("0")
            edit.setFixedWidth(55); edit.setFixedHeight(30); edit.setAlignment(Qt.AlignCenter)
            ...
            btn_mas = QPushButton("+"); btn_mas.setFixedSize(30, 30)
            btn_mas.clicked.connect(lambda ch, d=den: self.ajustar_conteo(d, 1))
            
            h_controles.addWidget(btn_menos); h_controles.addWidget(edit); h_controles.addWidget(btn_mas)
            grid.addWidget(lbl, i, 0)
            grid.addLayout(h_controles, i, 1)

        ly_desglose.addLayout(grid)
        self.lbl_total_conteo = QLabel("TOTAL CONTEO: Q 0.00")
        self.lbl_total_conteo.setStyleSheet("font-size: 14px; font-weight: 800; color: #111827; margin-top: 10px; border-top: 1px solid #EEE; padding-top: 10px;")
        ly_desglose.addWidget(self.lbl_total_conteo, alignment=Qt.AlignRight)
        grupo_desglose.setLayout(ly_desglose)

        # --- SECCIÓN DERECHA: FORMULARIOS ORIGINALES ---
        ly_derecho = QVBoxLayout()
        ly_derecho.setSpacing(15)

        # Apertura (Original)
        apertura_group = QGroupBox("Apertura de Caja")
        apertura_group.setStyleSheet(grupo_desglose.styleSheet())
        apertura_layout = QFormLayout()
        self.monto_inicial = QDoubleSpinBox()
        self.monto_inicial.setMinimum(0); self.monto_inicial.setMaximum(100000)
        self.monto_inicial.setPrefix("Q"); self.monto_inicial.setFixedHeight(35)
        ...
        cierre_group.setStyleSheet(grupo_desglose.styleSheet())
        cierre_layout = QFormLayout()
        self.monto_final = QDoubleSpinBox()
        self.monto_final.setMinimum(0); self.monto_final.setMaximum(1000000)
        self.monto_final.setPrefix("Q"); self.monto_final.setFixedHeight(35)
        ...
        layout_principal.addWidget(grupo_desglose, 2)
        layout_principal.addLayout(ly_derecho, 1)
        
        tab.setLayout(layout_principal)
        return tab

    # --- NUEVOS MÉTODOS PARA EL DESGLOSE ---
    def ajustar_conteo(self, den, delta):
        actual = int(self.inputs_efectivo[den].text() or 0)
        self.inputs_efectivo[den].setText(str(max(0, actual + delta)))

    def actualizar_monto_inicial_desde_conteo(self):
        total = sum(d * int(e.text() or 0) for d, e in self.inputs_efectivo.items())
        self.lbl_total_conteo.setText(f"TOTAL CONTEO: Q {total:,.2f}")
        # Asignamos al monto inicial de apertura
    ...
    def crear_tab_movimientos(self):
        ...
        form_group.setStyleSheet("font-weight: bold; border: 1px solid #E5E7EB; border-radius: 12px; margin-top: 12px; padding-top: 10px;")
        ...
        self.monto_mov.setMinimum(0); self.monto_mov.setMaximum(100000); self.monto_mov.setPrefix("Q")
    ...
    def cerrar_caja(self):
        ...
        if reply == QMessageBox.Yes:
            query = "UPDATE apertura_cierre SET fecha_hora_cierre = NOW(), monto_final = %s WHERE id_apertura = %s"
    ...
    def registrar_movimiento(self):
        ...
        if not descripcion or monto <= 0:
            QMessageBox.warning(self, "Error", "Complete los campos correctamente")
            return
        query = "INSERT INTO movimiento_caja (id_caja_fk, tipo_movimiento, descripcion, monto, fecha_hora) VALUES (%s, %s, %s, %s, NOW())"
        if self.db.execute_query(query, (self.id_caja_actual, tipo, descripcion, monto)):
            ...
            self.descripcion_mov.clear(); self.monto_mov.setValue(0)
            self.cargar_movimientos()

    def cargar_movimientos(self):
        if not self.id_apertura_actual: return
        query = """
            ...
            JOIN usuario u ON ac.id_usuario_fk = u.id_usuario
            WHERE ac.id_apertura = %s ORDER BY mc.fecha_hora DESC
        """
    ...
    def cargar_historial(self):
        query = """
            SELECT ac.fecha_hora_apertura, ac.fecha_hora_cierre, u.nombre, ac.monto_inicial, ac.monto_final
            FROM apertura_cierre ac JOIN usuario u ON ac.id_usuario_fk = u.id_usuario
            WHERE ac.fecha_hora_cierre IS NOT NULL ORDER BY ac.fecha_hora_apertura DESC
        """
        ...
        for i, h in enumerate(hist):
            ...
            self.historial_table.setItem(i, 1, QTableWidgetItem(str(h['fecha_hora_cierre'])[:19]))
            ...
            self.historial_table.setItem(i, 4, QTableWidgetItem(f"Q{h['monto_final']:.2f}"))
            diff = (h['monto_final'] - h['monto_inicial']) if h['monto_final'] else 0
            self.historial_table.setItem(i, 5, QTableWidgetItem(f"Q{diff:.2f}"))